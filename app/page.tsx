...
export default function Page() {
  ...
  // ui phase mirrors internal ref state
  ...
  useEffect(() => {
    ...
    function spawnObject() {
      ...
      const size = rf(6, 12);
      const speed = BASE_SPEED + score * SPEED_INC; // ok to read
        ...
        for (let i = objects.current.length - 1; i >= 0; i--) {
          ...
          if (o.y + o.size > p.y && o.x > p.x && o.x < p.x + p.w) {
            if (o.type === 'star') {
              ...
              // keep loop alive for overlay but stop spawning next frame
              // optional: set alive false if you want to freeze frame
            }
  ...
  function startCountdownThenPlay() {
    objects.current = [];
    setScore(0);
    state.current = 'countdown'; setUiPhase('countdown');
    setHint('Get Ready ✨');
    setCountdown(3);
    ...
    const timer = setInterval(() => {
      setCountdown((c) => {
        if (c <= 1) {
          clearInterval(timer);
          state.current = 'playing'; setUiPhase('playing');
          setHint('Catch stars and avoid rocks');
          return 0;
        }
        return c - 1;
      });
  ...
  function playClickGuard(): boolean {
    ...
    clickLockUntil.current = now + 1500;
    return true;
  }

  /** force unique tx and smart retries */
  async function payThenStart() {
    ...
    // build always unique eip1559 tx
    const buildTx = (attempt: number, withData: boolean) => {
      const bump = BigInt((Date.now() % 19) + 1 + attempt); // 1..19 + attempt
      const value = PRICE_WEI + bump;

      const baseTip = BigInt(100000000); // 0.1 gwei
      const jitter = BigInt(Math.floor(Math.random() * 30)) * BigInt(1000000); // up to 0.03 gwei
      const maxPriorityFeePerGas = baseTip + jitter;
      const maxFeePerGas = maxPriorityFeePerGas + BigInt(1000000); // +0.001 gwei
      ...
      if (withData) {
        ...
        req.data = stringToHex(memo) as Hex;
      }
      return req;
    };
    ...
    try {
      ...
      if (!inside) { setHint('Open inside Warpcast or Base app'); return; }
      ...
      if (!hash) { setHint('Could not create transaction'); return; }
